...
- [ ] In `notion.repo.ts`, add a function `getNotionDatabaseSchema(databaseId, notionToken)` that calls the Notion API to retrieve the database object and extracts all options from the `Status` property (or equivalent select/multi-select property).
- [ ] Return a list of raw status option names as strings.

#### 1.2 — Map Notion statuses to DevTrack TicketStatus enum

- [ ] In `notion.mapper.ts`, add a function `buildDefaultStatusMapping(notionStatuses: string[])` that takes the raw Notion status names and maps each one to the closest `TicketStatus` enum value using the existing default mapping logic.
- [ ] Return a `Record<string, TicketStatus>` where keys are Notion status names and values are the mapped DevTrack statuses.
- [ ] Unknown statuses that cannot be confidently mapped should default to `TODO`.

#### 1.3 — Add a new endpoint to fetch the pre-filled mapping

- [ ] In `notion.schema.ts`, add a schema for the request params (just `projectId`).
- [ ] In `notion.service.ts`, add `getDefaultStatusMappingService(projectId, organizationId)` that:
...
- [ ] In `notion.controller.ts`, add `getDefaultStatusMappingController`.
- [ ] In `notion.routes.ts`, register `GET /:projectId/status-mapping/default` with `requireAuthMiddleware` and `requireRoleMiddleware('TEAM_LEADER')`.

#### 1.4 — Verify

- [ ] Hit the new endpoint with a connected project and confirm it returns a pre-filled `Record<string, TicketStatus>`.
- [ ] Confirm unknown Notion statuses default to `TODO`.
...
- [ ] In `notion.mapper.ts`, update the ticket mapping logic to extract the `Module` or `Feature` property from each Notion page (check both names — the column may be called either).
- [ ] Add `moduleName: string | null` to the `SyncedTicketRecord` type.
- [ ] If the property does not exist or is empty, set `moduleName` to `null`.

#### 2.2 — Add feature upsert helper to feature repo

- [ ] In `feature.repo.ts`, add `upsertFeatureByName(projectId, name)` that:
...
- [ ] Deduplication must be case-insensitive.

#### 2.3 — Update sync worker to auto-assign features

- [ ] In `sync.worker.ts`, after fetching tickets from Notion and before persisting them, collect all unique non-null `moduleName` values from the synced ticket records.
- [ ] For each unique module name, call `upsertFeatureByName` to ensure the feature exists. Build a `Map<normalizedName, featureId>` lookup.
- [ ] When persisting each ticket, resolve its `featureId` from the lookup using its `moduleName`.
- [ ] If `moduleName` is null, leave `featureId` unchanged from its current value (do not unassign manually-assigned features).
- [ ] If `moduleName` is set, update `featureId` to the resolved ID — even if it was previously assigned to a different feature. This keeps DevTrack in sync with Notion on every run.

#### 2.4 — Verify

- [ ] Run a sync on a project where Notion tickets have a `Module` or `Feature` column.
- [ ] Confirm new features are auto-created with normalized names.
- [ ] Confirm tickets are assigned to the correct features.
- [ ] Confirm tickets without a module value are not unassigned.
- [ ] Confirm running the sync twice produces the same result (idempotency).
- [ ] Confirm the sync log still records correctly after the changes.

---

### Phase 3 — Cleanup and Memory

- [ ] Review all changed files against `STANDARDS.md` — no `any`, no default exports, no raw `process.env`, no `console.log`.
- [ ] Confirm Notion token is never logged or leaked in any new code path.
- [ ] Confirm all new Prisma writes that touch multiple rows are wrapped in transactions.
- [ ] Append key takeaways from this session to `MEMORY.md`.
